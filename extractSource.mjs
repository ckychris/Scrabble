...
const CURATED_WORDS = [
  ...
  '朋友', '家人', '父母', '兄弟', '姐妹', '老師', '學生', '醫生', '警察', '工人',
  '男人', '女人', '小孩', '老人', '同事', '鄰居', '太太', '先生', '女朋友', '男朋友',
  // Daily life
  '食飯', '飲水', '睡覺', '工作', '讀書', '行路', '坐車', '返家', '出門', '買嘢',
  '煮飯', '洗碗', '打掃', '休息', '運動', '睇書', '聽歌', '睇戲', '打電話', '上網',
  // Food & drink
  '早餐', '午餐', '晚餐', '麵包', '白飯', '炒飯', '麵條', '湯水', '咖啡', '奶茶',
  '豬肉', '牛肉', '雞肉', '魚類', '蔬菜', '水果', '點心', '燒味', '火鍋', '煲仔',
  // Places
  '香港', '學校', '醫院', '超市', '餐廳', '公園', '銀行', '辦公', '地鐵', '巴士',
  '機場', '酒店', '商場', '街市', '圖書', '教堂', '政府', '大學', '中學', '小學',
  // Time
  '今日', '聽日', '昨日', '早上', '下午', '晚上', '星期', '月份', '年頭', '節日',
  '時間', '分鐘', '小時', '週末', '假期', '生日', '新年', '聖誕', '中秋', '清明',
  // Emotions & states
  '開心', '傷心', '快樂', '難過', '緊張', '放鬆', '疲倦', '精神', '健康', '生病',
  '擔心', '高興', '生氣', '失望', '滿意', '驚訝', '害怕', '興奮', '感動', '舒服',
  // Common verbs
  '知道', '明白', '覺得', '希望', '需要', '決定', '開始', '完成', '幫助', '準備',
  '記得', '忘記', '相信', '答應', '拒絕', '選擇', '改變', '解決', '發現', '使用',
  // Adjectives
  '好靚', '好大', '好細', '好多', '好少', '好快', '好慢', '好難', '好易', '好貴',
  '便宜', '重要', '普通', '特別', '正常', '奇怪', '有趣', '無聊', '清楚', '複雜',
  // HK culture
  '廣東', '粵語', '普通', '繁體', '文化', '傳統', '社會', '經濟', '政治', '環境',
];
...
console.log(`Source dictionary saved → ${outPath} (${fs.statSync(outPath).size} bytes)`);
console.log('Sample:', result.slice(0, 10).map(w => `${w.v}(${w.j})`).join(', '));
console.log('Done!');
